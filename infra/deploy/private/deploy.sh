...
if [ -n "$OPENROUTER_API_KEY_SECRET_OCID" ]; then
  ...
  OPENROUTER_API_KEY_VALUE="$(get_vault_secret_by_name "$OPENROUTER_API_KEY_SECRET_NAME")"
fi
if [ -n "$OPENROUTER_API_KEY_SECRET_OCID" ] || [ -n "$OPENROUTER_API_KEY_SECRET_NAME" ]; then

...
- Az OpenRouter proxy alapertelmezett modellje `qwen/qwen3.6-plus`, mert text, image es video inputot is tud kezelni.
- Az OpenRouter proxy `OPENROUTER_ALLOWED_MODELS`, `OPENROUTER_MAX_COMPLETION_TOKENS` es `OPENROUTER_MAX_REQUEST_BYTES` guardrailekkel korlatozza az AI runtime koltseg- es payload-kockazatat, a default payload limit pedig kepes kerdesekhez is eleg teret ad.

...
- Private OCI host runs the Spring Boot backend and a lightweight OpenRouter provider proxy for AI calls.
...
- PR bodies must follow the template in `AGENT.md` and keep `Version` and `Verification` at the top.
...
- A privát OCI hoston fut a Spring Boot backend és egy könnyű OpenRouter provider proxy az AI hívásokhoz.

...
1. A `Container Images` workflow buildelje és pusholja a gateway image-et (`ghcr.io/<registry-owner>/autoforge/api-gateway`), vagy lokálisan buildeld az `infra/gateway` mappából.
2. Állítsd be a publikus host `.env`-jében a `BACKEND_UPSTREAM` változót a privát backend belső címére (pl. `<private-backend-ip>:8080`).
